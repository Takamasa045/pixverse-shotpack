...
2. PixVerse CLI はこのリポジトリの外で、事前にインストールしておきます。
3. 実行前に認証状態とアカウント情報を確認します。
...
4. 入力として `brief.md` か `storyboard.yaml` を用意します。
5. 最初に `SKILL.md`、次に `workflows/orchestrator-flow.md` を確認します。

このリポジトリ自体はドキュメントとワークフロー契約の置き場なので、`npm install` やローカル build 手順はありません。
